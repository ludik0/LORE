...
def main():
    ...
    print ("Number of Prediction:", len(y2E))
    y2E = np.asarray([dataset['possible_outcomes'][i] for i in y2E])

    idx_record2explain = 0
    for j in range(20):
        print("j:"+str(j))
        explanation, infos = lore.explain(j, X2E, dataset, blackbox,
        ...
        print ("Number of Explanations:", len(explanation))
        dfX2E = build_df2explain(blackbox, X2E, dataset).to_dict('records')
        dfx = dfX2E[j]
        x = build_df2explain(blackbox, X2E[j].reshape(1, -1), dataset).to_dict('records')[0]
