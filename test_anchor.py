...
def main():
    ...
    for j in range(20):
        ...
        print('Prediction: ', possible_outcomes[blackbox.predict(X2E[j].reshape(1, -1))[0]])

        exp, info = explainer.explain_instance(X2E[j].reshape(1, -1), blackbox.predict, threshold=0.95)
        ...
        fit_anchor = np.where(np.all(X2E[:, exp.features()] == X2E[j][exp.features()], axis=1))[0]
        ...
        print('Anchor test precision: %.2f' % (np.mean(blackbox.predict(X2E[fit_anchor]) ==
                                                    blackbox.predict(X2E[j].reshape(1, -1)))))
